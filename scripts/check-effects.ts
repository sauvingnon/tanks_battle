...
// --- 1б. Толчок от выстрела и попадания ---

{
  // Правило одно: вверх идёт та сторона, с которой пришла сила. Знаки тут
  // обратны ходовому крену — сила приложена выше центра тяжести, а не ниже, —
  // и перепутать их проще всего: наклон будет, просто не в ту сторону.
  const S = 0.05;

  // Выстрел вперёд: сила отдачи смотрит назад (-Z), нос задирается, то есть
  // поворот вокруг X отрицательный — тот же знак, что и у разгона.
  const forward = bodyKick(-1, 0, S);
  check('выстрел вперёд задирает нос', forward.pitch < 0);
  check('выстрел вперёд корпус не кренит', near(forward.roll, 0));
  check('выстрел назад роняет нос', bodyKick(1, 0, S).pitch > 0);

  // Выстрел на левый борт (+X): сила смотрит в -X, поднимается левый борт,
  // а это положительный поворот вокруг Z — как в повороте влево.
  const toLeft = bodyKick(0, -1, S);
  check('выстрел на левый борт поднимает левый', toLeft.roll > 0);
  check('выстрел на правый борт зеркален', near(bodyKick(0, 1, S).roll, -toLeft.roll));
  check('выстрел вбок не клюёт', near(toLeft.pitch, 0));

  // Попадание: сила смотрит от места удара внутрь танка. Прилетело в лоб —
  // сила идёт назад, и нос задирается ровно как от своего выстрела вперёд.
  check('попадание в лоб задирает нос', bodyKick(-1, 0, S).pitch < 0);
  check('попадание в корму роняет нос', bodyKick(1, 0, S).pitch > 0);
  check('сила толчка масштабируется', near(bodyKick(-1, 0, 2 * S).pitch, 2 * forward.pitch));

  // Толчок обязан быть заметно меньше позы: это вздрагивание, а не положение.
  check('толчок не спорит с ходовым креном', S < LEAN_MAX);
}
...

{
  ...
  // Текущая глубина просадки — 0.35 м: остов заметно садится, но не уходит
  // ниже своей серверной коллизии и не превращается в невидимую стену.
  check('к концу остов осел в воронку, но не исчез из виду', end <= -0.25 && end > -0.6);
