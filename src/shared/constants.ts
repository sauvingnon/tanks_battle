...
/**
 * Высота силуэта танка, м: от земли под ним до крыши башни. На плоскости она ни
 * на что не влияла — снаряд летел на постоянной высоте и попадал в круг, — но
 * с рельефом и вертикальной наводкой снаряд проходит над целью или под ней, и
 * попадание становится вопросом высоты.
 */
export const TANK_HEIGHT = 2.6;

/** Общий зажим. Здесь, а не в sim.ts, чтобы им пользовался и рельеф: sim от него зависит сам. */
...
/**
 * Ускорение свободного падения, м/с². Нужно ровно одному: склону под гусеницами.
 * Снаряд по-прежнему не падает — упреждение важнее баллистики.
 */
export const GRAVITY = 9.81;

/**
 * Пределы, в которых склон двигает потолок хода. Смысл множителя — какая доля
 * мотора остаётся на разгон после того, как часть тяги ушла на подъём; поэтому
 * ровное место — это ровно единица, и аркада ничего не замечает.
 *
 * Нижняя граница нужна не для вкуса, а для целости: на склоне круче, чем тянет
 * мотор, множитель ушёл бы в минус, и потолок хода стал бы отрицательным. Верхняя
 * держит разгон под гору в пределах разумного — на 43-градусном скате «Долины»
 * без неё танк разогнался бы до 75 км/ч.
 */
export const SLOPE_HOLD_MIN = 0.25;
export const SLOPE_HOLD_MAX = 1.6;

/**
 * Насколько выше земли танк считается оторвавшимся, м. Порог нужен только
 * численности: выборка поля и интегрирование высоты сходятся не побитово, и без
 * зазора танк на ровном месте моргал бы между «на земле» и «в воздухе».
 */
export const GROUND_EPS = 0.02;

/**
 * Сколько хода съедает приземление, в долях вертикальной скорости удара.
 * Прыжок должен стоить: иначе гребни превращаются в трамплины, по которым
 * выгодно гонять, и вся тяжесть, добытая склоном, пропадает даром.
 */
export const LANDING_DRAG = 0.35;

/**
 * Мягче этого касание землю не бьёт, м/с. Съезжая с пологой горки, танк
 * отрывается на сантиметры десятки раз подряд, и без порога он терял бы ход
 * непрерывно там, где визуально просто едет.
 */
export const LANDING_SOFT = 2.5;
...
/**
 * Пределы вертикальной наводки, рад: вниз и вверх. Числа взяты у настоящих
 * танков (-9° и +18°) и нужны ровно за тем же, за чем нужны там: пушка, которая
 * смотрит куда угодно, отменяет рельеф. Со склонением в 9 градусов возвышенность
 * даёт преимущество — сверху ты достаёшь того, кто снизу тебя уже не достаёт, —
 * а гребень перед стволом становится позицией, а не декорацией.
 *
 * На картах без рельефа наводка всегда 0: и предел, и сама ось там ни при чём.
 */
export const GUN_PITCH_MIN = -0.16;
export const GUN_PITCH_MAX = 0.31;
