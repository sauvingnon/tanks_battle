...
export const MAPS: MapDef[] = [
  ...
  },
  {
    ...
    half: BIG_HALF,
    // Вдвое шире и вдвое выше «Холмов» при том же уклоне: 0.62·amp·3/feature
    // и дальше по октавам даёт ту же крутизну склона, что там, — а холм при
    // этом вырастает с тридцати метров поперёк до шестидесяти. Именно это
    // отличает большую карту от маленькой: не квадратные метры, а то, что
    // гребень теперь длиннее дистанции боя.
    terrain: { seed: 20261005, amp: 8, feature: 60 },
  },
  {
    name: 'Промзона',
    build: buildWorks,
    spawns: WORKS_SPAWNS,
    half: BIG_HALF,
    // Рельеф здесь нарочно почти плоский: землю под цеха выровняли бы и в
    // жизни. Он не укрытие, а фон — но и не ноль, иначе карта ничем не
    // отличалась бы от аркадных, кроме размера.
    terrain: { seed: 20261012, amp: 2.4, feature: 44 },
  },
];
...
/** Карта целиком: блоки, земля под ними и её размер. */
...
export function buildScene(id = 0): MapScene {
  ...
  const terrain = def ? buildTerrain(def, half) : FLAT;
  settleBoxes(terrain, obstacles);
  return { obstacles, terrain, half };
}
