...
export function stepTank(
  ...
  // отклонён руль, а нос в противоположную сторону. Инверсия следует за нажатым
  // газом, а не за фактической скоростью: как только игрок жмёт «назад», руль сразу
  // работает по-заднеходному, даже если танк ещё катится вперёд по инерции. Без
  // газа (throttle === 0, накат) инверсия держится по текущей скорости — иначе на
  // выбеге руль внезапно дёргался бы обратно.
  const reversing = throttle !== 0 ? throttle < 0 : state.speed < 0;
  const wheel = reversing ? -steer : steer;
