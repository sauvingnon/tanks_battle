...
/**
 * Сколько идёт анимация оседания. Дальше остов не убирается: на сервере
 * подбитый танк стоит препятствием до возрождения (в волнах — до её конца), и
 * машина всё это время должна оставаться на виду — иначе получится невидимая
 * стена. WRECK_S — только длительность просадки в первые секунды после гибели.
 */
export const WRECK_S = 1.7;
/** Пауза перед оседанием: столько остов просто горит на месте. */
const WRECK_HOLD_S = 0.4;
/**
 * На сколько метров осевший остов уходит под землю. Корпус с башней — около
 * 2.5 м, но это не «спрятать», а «вмять в воронку»: остов должен читаться
 * издали весь бой, поэтому глубина заметно меньше собственной высоты танка.
 * Хитбокс на сервере не проседает вместе с моделью и держит полную высоту
 * до возрождения — глубина держится небольшой, чтобы остов не выглядел
 * ниже своей же невидимой стены (снаряд рядом не должен бить в пустоту).
 */
const WRECK_DEPTH = 0.35;
...
/** Ширина отпечатка чуть шире звена, чтобы колея не выглядела рваной. */
export const TRACK_WIDTH = 0.58;
/** Длина отпечатка чуть больше шага эмиссии: соседние отпечатки стыкуются. */
export const TRACK_LENGTH = 0.9;
